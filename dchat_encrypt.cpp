...
int main(int argc, char* argv[]){
	...
	if(argc==3){
		...
		mydetails.isLead = false;//Change 2
...
void socketAction(char ip[],int port,bool status){
	...
	if(status == true){
		...
		for(std::list<sockaddr_in>::iterator it=socketList.begin(); it!=socketList.end(); it++){
			...
			cout << "checking " << string(addressBuffer) <<":"<<to_string(socklist_port) << endl;
			
			if( socklist_port == port && (strcmp(addressBuffer, ip) == 0 )   ){
				cout << "Found to delete" << string(ip) <<":"<<socklist_port << endl;
				to_delete = it;
				found = true;
			}
...
void *receiveMessage(void *){
	...
	while(1){
		...
		//cout << "Current sequencer number: "<< to_string(currentSequenceNumber) << endl;
 	...
 	while(1){
		...
		if(checkLeader == true){
			...
			for (std::list<sockaddr_in>::const_iterator it = socketList.begin(), end = socketList.end(); it!= end; ++it) {
				...
				// increment port +5, we are sending to heartbeatRecv socket of participants
...
void *heartbeatReceive(void *){
	...
	while(1){
		...
		if( checkLeader == true){
			...
			if(recv_bytes > 0){
				cout<<"received hbt msg type: "<<to_string(aliveMsg.message_type)<<endl;
			...
			if(recv_bytes>0){
				...
				if(aliveMsg.message_type == ELECTION){
					...
					if(highestIpPort){
						...
						pthread_mutex_unlock(&socketList_mutex);
						//Should we update the global leader details.
						checkLeader = true;	
				...
				//Have to keep a timeout to receive a OK message. If not received in a particular timeout I will elect myself as a leader.
				else if(aliveMsg.message_type == OK){
					
					cout << "waiting for leader to be elected" << endl;
					...
					cout << "Recieved Leader message " << endl;
			...
			if(recv_bytes < 0){
				//start election
				cout<<string(ld.name)<<" was the sequencer and it died on "<<string(ld.ip)<<":"<<to_string(ld.port)<<endl;
				cout<<"Election has Started"<<endl;
				sleep(3);
				...
				string deleteheartBeatKey = string(ld.name)+":"+string(ld.ip)+":"+to_string(ld.port);//string(ld.name)+":"+
...
void *printMessage(void *){
	...
	while(1){
		...
		if(printMsg.message_type == SEQ_CHAT && !checkLeader){
